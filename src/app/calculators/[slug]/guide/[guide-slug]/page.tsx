...
export async function generateMetadata({
    ...
    params: Promise<{ slug: string; 'guide-slug': string }>
}): Promise<Metadata> {
    const { slug, 'guide-slug': guideSlug } = await params
    const data = getCalculatorBySlug(slug)
    const guide = data?.guide?.find((item) => item.id === guideSlug)
    ...
    const canonical = data?.SEO_canonical
        ? `${data.SEO_canonical}/guide/${guideSlug}`
        : `/calculators/${slug}/guide/${guideSlug}`
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-7 gap-5 p-5 md:p-10 mx-auto">
            <div className="col-span-1 md:col-span-5 flex flex-col gap-6">
                <h1 className="text-5xl pl-10 font-bold">{guide?.title || "Guide"}</h1>

                <MarkdownViewer filePath={`/public/markdown/guide/${slug}/${guideSlug}.md`} />
